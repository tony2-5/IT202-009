...
// first query to select order table data and the Users id from users table
$base_query = "SELECT Orders.id, Orders.created, Orders.money_recieved, Orders.address, Users.username
FROM Users JOIN Orders on Users.id = Orders.user_id";
$total_query = "SELECT count(1) as total FROM Users JOIN Orders on Users.id = Orders.user_id";
//dynamic query
...
if (!empty($startDate) && !empty($endDate)) {
    $query .= " AND Orders.created BETWEEN :startDate AND :endDate";
...
if (!empty($col) && !empty($orderSort)) {
    $query .= " ORDER BY Orders.$col $orderSort"; //be sure you trust these values, I validate via the in_array checks above
}
    ...
    <table class="table table-striped">
        ...
        <?php foreach ($order as $o) : ?>
            ...
            <tr>
                ...
                <?php $totalPageValue += se($o, "money_recieved","",false)/100?>
                <td><a href="../orderconfirmation.php?id=<?php se($o, "id"); ?>">Click to view more details</a></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
        <tr>
            <td>Value Of Orders On Page: $<?php se($totalPageValue); ?></td>
        </tr>
    </table>
    <div class="mt-3">
